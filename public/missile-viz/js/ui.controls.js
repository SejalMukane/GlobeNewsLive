...
var d3Graphs = {
	...
	histogramHeight: 160,
	histogramBarWidth: [21, 12],
	histogramLeftPadding: [32.5, 22],
	histogramRightPadding: [32.5, 22],
	...
	histogramOpen: false,
	handleLeftOffset: [35, 18],
	handleInterval: [27, 14],
	...
	showHud: function() {
		...
		for (var i in missileLookup) {
			// Escape special characters for jQuery selector (apostrophes, parentheses, etc.)
			var safeClass = i.replace(/[()'"\[\]]/g, '\\$&');
			try {
				var btn = $(".missileTypeBtn." + safeClass + " .label");
				if (btn.length) {
					btn.text(missileLookup[i].name || missileLookup[i].description || i);
				}
			} catch(e) {
				console.warn('⚠️ Could not update button for missile:', i);
			}
		}
...
//		var windowWidth = $(window).width();
	...
	drawBarGraph: function() {
		...
		var max = Math.max(successTotal, failureTotal, unknownTotal, 25);
